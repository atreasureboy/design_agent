...
- 六类问题形态会推荐模板与 Runtime；推荐只是解释充分的起点，用户仍可偏离选择
- 模板元数据补齐适用场景、预置能力与关键代价，不再只显示一句营销式描述
- 创建时直接采集业务结果、关键用例、利益相关者、验收标准、数据分级、自治程度、硬约束、信任边界、合规与 NFR
- 所有立项答案随创建请求写入 `ArchitectureBrief`，架构助手进入蓝图后可立即给出场景化补全建议
- 右侧实时形成 Design Brief 摘要，并对敏感数据和高自治场景提前提示治理重点
...
- 所有 Web 与 MCP 蓝图写入都会在共享数据目录追加持久化变更事件，跨进程也不会丢失通知
